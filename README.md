...
- ⚙️ **配置管理** - 使用 TOML 配置文件，支持 Web 在线编辑和热加载
...
🔥 **热加载支持**：修改配置文件后会自动重新加载，无需重启应用！支持的热加载配置包括：
- API 配置（密钥、模型、地址）
- 内存配置（历史消息数、压缩阈值）
- 调度器配置（定时任务、主动聊天）
- 角色配置（添加/删除/修改角色）
...
详细说明请查看 [热加载文档](docs/HOT_RELOAD.md)。

⚠️ **注意**：保存时会自动备份当前配置到 `config.toml.backup.时间戳`。
...
📖 **详细说明**：[群聊功能文档](docs/GROUP_CHAT_FEATURES.md)
...
📖 **详细说明**：[多群隔离机制](docs/MULTI_GROUP_EXPLANATION.md)

## 🎮 Discord 使用指南

Discord 平台有特殊的使用方式，推荐通过频道权限来区分私聊和群聊：

### 私聊设置（私密频道）

创建私密频道（@everyone 无法查看）：
- Bot 会将私密频道当作"私聊"
- 每条消息都会回复
- 可以说私密的话
...
📖 **详细说明**：[Discord 使用指南](docs/DISCORD_USAGE.md)

## 📖 使用文档

- 📘 [快速开始](QUICKSTART.md) - 5分钟快速上手
- 🎮 [Discord 使用指南](docs/DISCORD_USAGE.md) - Discord 私聊和群聊设置
- 👥 [群聊功能说明](docs/GROUP_CHAT_FEATURES.md) - 群聊响应规则详解
- 🔒 [多群隔离机制](docs/MULTI_GROUP_EXPLANATION.md) - 多群聊隔离原理
- 🌐 [Web 界面说明](docs/WEB_INTERFACE.md) - Web 管理界面使用
- 🎭 [创建角色](docs/CREATE_CHARACTER.md) - 如何创建新角色
...
- ✅ **Discord 私聊/群聊区分**
  - 通过频道权限自动识别私密频道和公开频道
  - 私密频道（@everyone 不可见）→ 当作私聊，每条消息都回复
  - 公开频道（@everyone 可见）→ 当作群聊，使用智能判断逻辑
  - 支持通过删除频道来清空聊天记录
  - 新增 Discord 使用指南文档

- ✅ **群聊响应逻辑优化**
  - 修复"你们俩"等群体称呼响应概率过低的问题（20% → 75%）
  - 新增 'high' 优先级，提升群体称呼的响应率
  - 修复 Telegram bot 中错误的保护逻辑导致判断结果被忽略
  - 优化群聊判断逻辑，更符合实际使用场景

- ✅ **时间识别修复**
  - 修复 AI 无法正确识别当前时间的问题
  - 只在最后一条用户消息添加时间戳，避免历史时间戳混淆
  - 优化系统提示中的时间说明
  - AI 现在能正确判断早上、中午、下午、晚上

- ✅ **其他优化**
  - 屏蔽 punycode 弃用警告
  - 修复 Discord.js 的 ready 事件弃用警告（改用 clientReady）
  - 移除调试日志，减少日志噪音
  - 优化 Web 界面对话管理页面，更紧凑节省空间

**文档更新**

- 新增 `docs/DISCORD_USAGE.md` - Discord 使用指南
- 更新 README，添加文档链接部分
- 完善群聊功能说明
